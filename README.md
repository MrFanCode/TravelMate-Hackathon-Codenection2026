...
**Team:** MUHAMMAD IRFAN BIN DHAJUDEEN, NUR AMALINA QISTINA BINTI MOHD YUNOS, NURUL ALWANI BINTI MOHD ARIF
...
Walkthrough of all 15+ prototype screens, grouped by the order a user actually moves through them. Images live in `/screens`.

**Onboarding**

| Welcome | Sign Up | Log In |
|---|---|---|
| ![Welcome](screens/welcome.png) | ![Sign Up](screens/signup.png) | ![Log In](screens/login.png) |
| Sets the "passport & airmail" identity and one-line value prop | Google OAuth or email/password — no verification needed to try the prototype | Returning-user entry point, same auth options |

**Trip Setup (3-step wizard)**

| Step 1 — Where & When | Step 2 — Interests | Step 3 — Solo or Group |
|---|---|---|
| ![Trip setup: destination, dates, budget](screens/trip-setup-1-destination.png) | ![Trip setup: interests](screens/trip-setup-2-interests.png) | ![Trip setup: solo or group](screens/trip-setup-3-solo-or-group.png) |
| Destination, dates, and a budget slider, with flight/hotel picks surfaced inline from mocked data | Pick a few interest tags — shapes how the AI itinerary engine builds each day | The one fork in the whole app: solo goes straight to itinerary, group goes through group setup first |

**Group Sync Flow** *(only for Group trips)*

| Invite | Calendar Check | Vote | Compromise |
|---|---|---|---|
| ![Bring your crew — invite link](screens/group-invite.png) | ![Who's actually free — Google Calendar check](screens/group-calendar-check.png) | ![Vote on Day 3](screens/group-voting.png) | ![Compromise found](screens/group-compromise.png) |
| Shareable link + QR code; anyone who joins can vote | Mocked Google Calendar free/busy check across the group before dates get locked in | Live vote bars as members pick between proposed activities for a given day | The Preference Compromise Engine turns vote counts into an actual split plan, not just a tally |

**Main App (tab bar: Trip / Map / Budget / Group)**

| Trip Dashboard | Map | Budget | Checklist |
|---|---|---|---|
| ![Your trip — itinerary timeline](screens/trip-dashboard.png) | ![Map with pinned stops and group members](screens/map.png) | ![Budget ring chart and category breakdown](screens/budget.png) | ![Personal checklist](screens/checklist.png) |
| Day-by-day itinerary timeline generated by the AI/OR-Tools layer | Real embedded OpenStreetMap with itinerary pins, nearby suggestions, and (group trips) live member locations | Spend-vs-budget ring chart plus a per-category breakdown (food, lodging, activities, transport) | AI-generated starter list adapted to destination/season, with manual add-your-own |

**Disruption → Adaptive Replan** *(core differentiator)*

| Disruption Alert | Replan Applied |
|---|---|
| ![Flight delayed disruption alert](screens/disruption-alert.png) | ![Day 1 updated after replan](screens/adaptive-replan.png) |
| A flight delay (or weather shift) surfaces what it breaks in today's plan | One tap regenerates the affected day around the new landing time — unaffected days are left alone |

**Safety, Profile & Offline**

| SOS | Profile & My Trips | Offline Export |
|---|---|---|
| ![Rosa needs help — SOS screen](screens/sos.png) | ![Profile and My Trips list](screens/profile-my-trips.png) | ![Offline PDF-ready trip copy](screens/offline-export.png) |
| Manual "I need help" alert with a teammate's shared live location | Account info plus trips in every state (planning/upcoming/completed/cancelled) — a second trip doesn't require closing the first | Itinerary, checklist, and budget collapsed into one printable page — real browser print-to-PDF, not a placeholder button |
